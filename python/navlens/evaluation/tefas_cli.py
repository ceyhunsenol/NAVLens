...
from .manifests import build_tefas_run_manifest, store_run_manifest
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        stored_manifest = store_run_manifest(manifest, arguments.run_root)
    except (OSError, TefasSourceError, ValueError) as error:
        print(f"error: {error}", file=sys.stderr)
        return 1
    print(format_tefas_backtest_result(result, stored_manifest))
    return 0
