...
def _contribution() -> PointInTimeReturnContributionResult:
    ...
    security_prices = [
        _security_price(PERIOD_START, 100.0),
        _security_price(PERIOD_END, 110.0),
    ]
    alignment = align_point_in_time(request, [holdings], security_prices)
...
def _security_price(market_date: MarketDate, price: float) -> SecurityPriceSnapshot:
...
def test_reconciles_exact_period_and_preserves_provenance() -> None:
    ...
    result = reconcile_point_in_time_fund_return(
        contribution,
        [start_snapshot, end_snapshot],
        fund_price_source_id="tefas",
    )

    assert isinstance(result, PointInTimeFundReturnReconciliationResult)
    ...
    assert reconciliation.reconciliation_residual == pytest.approx(0.02)
...
def test_selects_the_latest_visible_correction() -> None:
    ...
    assert reconciliation.reconciliation_residual == pytest.approx(0.05)
...
def test_rejects_unexpected_native_return_cardinality() -> None:
    ...
    with patch(
        "navlens.reconciliation.orchestration.calculate_price_period_returns",
        return_value=[],
    ):
...
def test_delegates_both_financial_operations_to_native_bindings() -> None:
    ...
    with (
        ...
        patch(
            "navlens.reconciliation.orchestration.reconcile_fund_return",
