"""Error, boundary, and fail-fast tests for source-backed historical FX builder."""
...
def test_decreasing_period_fails_before_any_provider_io() -> None:
    ...
    req1 = make_fx_historical_request(MarketDate(2026, 1, 2), MarketDate(2026, 1, 3), tz1)
    req2 = make_fx_historical_request(MarketDate(2026, 1, 1), MarketDate(2026, 1, 2), tz2)

    source = FakeRecordingSecurityPriceSource()
    holdings = [
        make_holding_snapshot(MarketDate(2026, 1, 1), tz1, (make_equity_position("AAPL", 1.0),))
    ]
    fund_prices = [make_fund_unit_price_snapshot(MarketDate(2026, 1, 1), 10.0, tz1)]
...
def test_duplicate_period_fails_before_any_provider_io() -> None:
    ...
    req1 = make_fx_historical_request(MarketDate(2026, 1, 1), MarketDate(2026, 1, 2), tz)
    req2 = make_fx_historical_request(MarketDate(2026, 1, 1), MarketDate(2026, 1, 2), tz)

    source = FakeRecordingSecurityPriceSource()
    holdings = [
        make_holding_snapshot(MarketDate(2026, 1, 1), tz, (make_equity_position("AAPL", 1.0),))
    ]
    fund_prices = [make_fund_unit_price_snapshot(MarketDate(2026, 1, 1), 10.0, tz)]
...
def test_duplicate_holding_instruments_produce_single_query_then_fail_in_rust() -> None:
    ...
    holdings = [
        make_holding_snapshot(
            MarketDate(2026, 1, 1),
            tz,
            (make_equity_position("AAPL", 0.5), make_equity_position("AAPL", 0.5)),
        )
    ]
    source = FakeRecordingSecurityPriceSource(
        data={
    ...
    assert source.queries[0].instrument_id == "AAPL"
...
def test_source_unavailable_propagates_fail_fast() -> None:
    ...
    source = FakeRecordingSecurityPriceSource(
        errors={"AAPL": SecurityPriceSourceUnavailableError("source offline")}
    )
    fund_prices = [make_fund_unit_price_snapshot(MarketDate(2026, 1, 1), 10.0, tz)]
...
def test_source_corrupted_data_propagates_fail_fast() -> None:
    ...
    source = FakeRecordingSecurityPriceSource(
        errors={"AAPL": SecurityPriceCorruptedSourceDataError("corrupted schema")}
    )
    fund_prices = [make_fund_unit_price_snapshot(MarketDate(2026, 1, 1), 10.0, tz)]
